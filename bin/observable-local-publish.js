...
commander
  .option('-o, --outputDir <directory>', 'directory into which the bundle will be published (.js files)', './dist/%NOTEBOOK_NAME%')
  .option('-n, --notebooks <directory>', 'directory from which to read the notebook', './notebooks')
  .option('-v, --views <directory>', 'directory from which to serve notebook views (.json files)', './notebook-views')
  .option('-p, --public <directory>', 'directory to serve as static -- in case your notebook might need to load files')// TODO: copy these files
  .action(async function() {
    ...
    if (fs.existsSync(path.resolve(notebookName + '.js'))) {
      ...
      notebookName = notebookName.split('/').pop()
    }
    ...
    try {
      ...
      const outputDir = path.resolve(commander.outputDir.replace('%NOTEBOOK_NAME%', notebookName))
      ...
      console.log(chalk.green(`Published to ${chalk.bold(outputDir)}`))
    } catch (e) {
